...
public class DefaultConfigProviderResolver extends ConfigProviderResolver implements ConfigRepository {
...
//    }
    ...
    @Override
    public <T> T get(String key, ClassLoader cl, Class<T> c) {
        Config config = this.getConfig(cl);
        if (config == null) {
            return null;
        }
        return config.getValue(key, c);
    }
}
